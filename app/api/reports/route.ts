...
import { renderReportPdfBuffer } from '@/lib/pdfReport';
...
export async function POST(request: Request) {
  ...
  // Fetch all matching items (single big page for now; chunk later if needed)
  const { items, total } = await listItems(household.id, filters);

  // Resolve location names for the report
